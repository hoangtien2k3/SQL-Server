...
*/
	-- DSSinhVien.HoSV + ' ' + DSSinhVien.TenSV AS HoTen : gộp Họ và Tên lại với nhau cách nhau ' '
	-- 'INNER JOIN' có thể viết tắt thành 'JOIN'
	SELECT DSSinhVien.HoSV + ' ' + DSSinhVien.TenSV AS HoTen, 
		   DSSinhVien.NgaySinh,
		   DSSinhVien.HocBong
	FROM DSSinhVien
	INNER JOIN KetQua				-- INNER JOIN để kết nối bảng SinhVien với bảng KetQua
	ON DSSinhVien.MaSV = KetQua.MaSV
	ORDER BY DSSinhVien.NgaySinh ASC, KetQua.Diem DESC; -- thực hiện cách điều kiện
...
*/
	/*
		-- xem câu lệnh query và ta tự hỏi sao lại cần có inner join là gì ?
		inner join có tác dụng kết nối các bảng lại với nhau, hoặc để lấy ra thông tin chính xác không bị sai lệch
		trong quá trình query dữ liệu từ table. Tránh tình trạng lấy không đúng, hoặc lấy sai dữ liệu
	*/
	-- cách 1:
	SELECT HoSV + ' ' + TenSV AS [Họ tên], DSSinhVien.MaKhoa AS [Mã khoa], NoiSinh AS [Nơi sinh], HocBong AS [Học bổng]
	FROM DSSinhVien
	JOIN DMKhoa ON DSSinhVien.MaKhoa = DMKhoa.MaKhoa
	WHERE HocBong >= 15000 AND NoiSinh = N'Hà Nội'
...
*/
	SELECT HoSV + ' ' + TenSV AS [Họ Tên], 
		DATEDIFF(YEAR, NgaySinh, GETDATE()) AS [Tuổi],  -- hàm này tính ra số tuổi của người đó
		TenKhoa AS [Tên Khoa]
	FROM DSSinhVien 
	JOIN DMKhoa 
...
*/
	SELECT MaSV, Phai, DSSinhVien.MaKhoa, 
		...
		END AS [Mức học bổng]
	FROM DSSinhVien
	WHERE HocBong IS NOT NULL;  -- is not null: có nghĩa là học bổng hiện tại không được phép null
...
*/
	...
	-- Tống sinh viên nữa:
	SELECT COUNT(*) AS [Tổng sinh viên nữ]
	FROM DSSinhVien
	GROUP BY Phai
	HAVING Phai = N'Nữ'
...
*/
	SELECT MaKhoa, COUNT(*) AS [Tổng sinh viên]
	FROM DSSinhVien
	GROUP BY MaKhoa
...
*/
	SELECT MaKhoa AS [Mã Khoa], 
		SUM(CASE WHEN Phai = N'Nam' THEN 1 ELSE 0 END) AS [Tổng Nam],
		SUM(CASE WHEN Phai = N'Nữ' THEN 1 ELSE 0 END) AS [Tổng Nữ]
	FROM DSSinhVien
	GROUP BY MaKhoa
...
/*
19.	Đưa ra điểm trung bình của sinh viên có mã ‘004’
*/
	SELECT AVG(KetQua.Diem) AS DiemTrungBinh
	FROM KetQua 
	WHERE MaSV = '004'
...
*/
	-- câu này cũng hay (biết inner join các bảng)
	SELECT DSSinhVien.MaSV, DSSinhVien.HoSV, DSSinhVien.TenSV, DSSinhVien.NgaySinh, DMKhoa.TenKhoa, 
		   AVG(KetQua.Diem) AS DiemTrungBinh
	FROM DSSinhVien
	INNER JOIN DMKhoa ON DSSinhVien.MaKhoa = DMKhoa.MaKhoa
